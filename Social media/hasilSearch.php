...
<body>
	...
	<?php 
			...
			$sqlc = "select * from comment where hashtag like binary '%$search%'";
		...
		//mengambil data post jika searh sama persis atau tidak
		$sql = "select * from post where hashtag like binary '%$search%'";
		$sqlc = "select * from comment where hashtag like binary '%$search%'";
		if ($search == "") {
	...
	<div class="post">
		...
		<?php 
			//mengambil data comment comment yang berada di id post ini
			$sqlc = "select * from comment where postid ='$id' or hashtag like binary '%$search%'";
			$queryc = mysqli_query($conn, $sqlc);
			$numrowsc = mysqli_num_rows($queryc);
			for ($a=1; $a <= $numrowsc ; $a++) { 
				$resultc = mysqli_fetch_array($queryc);				
